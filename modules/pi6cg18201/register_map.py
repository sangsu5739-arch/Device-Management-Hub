"""
PI6CG18201 clock generator - register map definitions

Defines 8-byte register structure based on datasheet (DS39996 Rev 6-2).
"""
...
# SMBus 7-bit address is 0x68 or 0x6A depending on SADR pin
SLAVE_ADDRESS_7BIT_SADR_LOW: int = 0x68
SLAVE_ADDRESS_7BIT_SADR_HIGH: int = 0x6A

TOTAL_BYTES: int = 8

# Datasheet power-up defaults (reference)
...
REGISTER_FIELDS: List[BitField] = [
    # Byte 0
    BitField("BYTE0_RSVD_7_3", "Byte0 reserved bits [7:3] (do not change)", 0, 7, 3, read_only=True),
    BitField("OE_Q1", "Q1 output enable (Q1+/Q1-)", 0, 2, 2, {0: "Disabled", 1: "Enabled"}),
    BitField("OE_Q0", "Q0 output enable (Q0+/Q0-)", 0, 1, 1, {0: "Disabled", 1: "Enabled"}),
    BitField("BYTE0_RSVD_0", "Byte0 reserved bit [0] (do not change)", 0, 0, 0, read_only=True),
    # Byte 1
    BitField(
        "SS_READBACK",
        "Spread Spectrum pin state readback [7:6]",
        1,
        7,
        6,
    ...
    BitField(
        "SS_SW_CTRL",
        "Spread Spectrum SW control select [5] (0=Pin, 1=SW)",
        1,
        5,
        5,
    ...
    BitField(
        "SS_MODE",
        "Spread Spectrum SW mode [4:3]",
        1,
        4,
        3,
        {0: "SS Off", 1: "-0.25%", 2: "Reserved", 3: "-0.5%"},
    ),
    BitField("BYTE1_RSVD_2", "Byte1 reserved bit [2] (do not change)", 1, 2, 2, read_only=True),
    BitField(
        "AMPLITUDE",
        "Output amplitude select [1:0]",
        1,
        1,
        0,
    ...
    # Byte 2
    BitField("BYTE2_RSVD_7_3", "Byte2 reserved bits [7:3] (do not change)", 2, 7, 3, read_only=True),
    ...
    BitField("BYTE2_RSVD_0", "Byte2 reserved bit [0] (do not change)", 2, 0, 0, read_only=True),
    # Byte 3
    BitField(
        "REF_SLEW",
        "REF slew rate [7:6]",
    ...
    BitField("REF_OE", "REF output enable [4]", 3, 4, 4, {0: "Disabled", 1: "Enabled"}),
    BitField("BYTE3_RSVD_3_0", "Byte3 reserved bits [3:0] (do not change)", 3, 3, 0, read_only=True),
    # Byte 4~6
    BitField("BYTE4_RSVD", "Byte4 reserved bits [7:0] (do not change)", 4, 7, 0, read_only=True),
    BitField("DEV_ID_HIGH", "Device ID high byte", 5, 7, 0, read_only=True),
    BitField("DEV_ID_LOW", "Device ID low byte", 6, 7, 0, read_only=True),
    # Byte 7
    BitField("BYTE7_RSVD_7_5", "Byte7 reserved bits [7:5] (do not change)", 7, 7, 5, read_only=True),
    BitField("BYTE_COUNT", "Read-back byte count [4:0]", 7, 4, 0),
]
...
class RegisterMap(QObject):
    ...
    def get_byte(self, index: int) -> int:
        if not 0 <= index < TOTAL_BYTES:
            raise IndexError(f"Byte index out of range: {index}")
    ...
    def set_all_bytes(self, data: bytes, emit: bool = True) -> None:
        if len(data) != TOTAL_BYTES:
            raise ValueError(f"Data length mismatch: {len(data)} (expected {TOTAL_BYTES})")
    ...
    def set_field(self, field_name: str, value: int, emit: bool = True) -> None:
        bf = FIELD_BY_NAME[field_name]
        if bf.read_only:
            raise PermissionError(f"Read-only field: {field_name}")
        max_val = (1 << bf.width) - 1
        if not 0 <= value <= max_val:
            raise ValueError(f"Value out of range: {value} (max {max_val})")
        raw = self._data[bf.byte_index]
    ...
    def spread_spectrum(self) -> int:
        # Compatibility: 0=Pin/Strap, 1=-0.25%(SW), 2=Reserved(SW), 3=-0.5%(SW)
    ...
    def slew_rate_coarse(self) -> int:
        # Compatibility: [Q1:Q0] 2-bit composite
    ...
    def slew_rate_coarse(self, value: int) -> None:
        if not 0 <= value <= 3:
            raise ValueError(f"Value out of range: {value} (max 3)")
